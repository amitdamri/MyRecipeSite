  ...
  <b-container>
    ...
    <b-row v-for="(n, i) in Math.floor(recipes.length / 5)" :key="i">
      <b-col class="col" v-for="(n, j) in 5" :key="j">
        <RecipePreview class="recipePreview" :recipe="recipes[i*5 + j]" :width="14.9" :height="30" />
      </b-col>
    </b-row>
    </div>
    <div v-if="recipes.length % 5 != 0">
      <b-row>
        <b-col class="col" v-for="(n, i) in recipes.length % 5" :key="i">
          <RecipePreview class="recipePreview" :recipe="recipes[5 * Math.floor(recipes.length / 5) + i]" />
        </b-col>
      </b-row>
    </div>  
    </div>
